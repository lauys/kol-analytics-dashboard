-- Add is_hidden column to kols table for soft delete functionality
-- Hidden KOLs are not deleted but hidden from regular users
-- Only admins can see and manage hidden KOLs

ALTER TABLE public.kols 
ADD COLUMN IF NOT EXISTS is_hidden boolean DEFAULT false;

-- Create index for faster filtering
CREATE INDEX IF NOT EXISTS idx_kols_is_hidden ON public.kols(is_hidden);

-- Update leaderboard view to include is_hidden field
...
COMMENT ON COLUMN public.kols.is_hidden IS 'Whether the KOL is hidden from regular users. Hidden KOLs are only visible to admins.';
